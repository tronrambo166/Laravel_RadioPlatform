...
{{--<a href="{{route('reportMDN')}}" class="rounded-0 px-4 btn btn-outline-light font-weight-bold my-1">Graph</a>--}}
 ...
 <table class="shadow mb-3 w-100 bg-white table tabil">
  <thead>
    <tr class=" bg-dark w-100">
      <h3 class="text-center bg-light text-dark py-1">Mdundo</h3>
...
<div class="col-sm-1"> </div>
  ...
  <tbody id="songs"> 
   ...
   @if(isset($rank)) <tr id="loading">
       <td scope="row" class="text-center"> ({{ $rank}}) </td>
   
      <td scope="row" class="text-center"> {{$lis}} </td>
      <td scope="row" class="text-center"> ${{round( 0.034*((int) filter_var($lis, FILTER_SANITIZE_NUMBER_INT)) ); }} </td>
